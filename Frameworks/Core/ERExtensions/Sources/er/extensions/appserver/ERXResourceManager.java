    ...
    private static WODeployedBundle _bundleWithNSBundle(NSBundle nsBundle)
    {
        ...
        if(aProjectBundle != null)
        {
            if(WOApplication._isDebuggingEnabled())
            	ERXResourceManager.log.debug((new StringBuilder()).append("Framework project found: Will locate resources in '").append(aProjectBundle.bundlePath()).append("' rather than '").append(aDeployedBundle.bundlePath()).append("' .").toString());
